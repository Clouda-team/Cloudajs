module.exports = function(baseDir, dstDir){
    /**
     * build file
     */
    if (!dstDir){
...
//判断build的文件是否是fw本身的文件
    	...
    	for(var i = 0; i < list.length; i++) {
    	    if(list[i].indexOf('package') === -1){
        		...
        		if(basename == "." || basename == ".."
        		  ||(basename.indexOf('.') === 0) //过滤.DS_STORE .svn .git这类文件夹和.profile这类文件
        		 ) {
        		    // pass these files
        		} else if(stat.isDirectory()) {
        		    // read dir recursively
	...
	if(!fs.existsSync(viewBaseDir)){
		...
		fs.exists(dircetoryPath, function(exists){
		    if(exists){
			fs.readFile(dircetoryPath, 'utf-8', function(error, entireContent){
			    if(error){
				console.log(dircetoryPath);
				...
				var currentPageToken = path.basename(dircetoryPath).split('.')[0];
